...
void Client::Recv()
{
	...
	if (ret > 0 && ret < sizeof(data))
	{
		...
		for (int i = 0; i < stoi(playerNum); i++)
		{
			...
			{
				(pStr[0] == "0"),//job
				std::stoi(pStr[i].substr(1,  4)),//x : ÇQï∂éöñ⁄Ç©ÇÁÇSï∂éö
				std::stoi(pStr[i].substr(5,  3)),//y : ÇSï∂éöñ⁄Ç©ÇÁÇRï∂éö
				std::stoi(pStr[i].substr(8, 1)), //state : ÇVï∂éöñ⁄Ç©ÇÁÇPï∂éö
				std::stoi(pStr[i].substr(9, 4))  //ID : ÇWï∂éöñ⁄Ç©ÇÁÇSï∂éö
			};
			playerList[i] = pData;
		}
	}
